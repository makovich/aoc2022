...
abdefghi"
    ...
    let (m, s, e) = hmap
        .lines()
        .fold((vec![], vec![], (0, 0)), |(mut acc, mut s, mut e), val| {
            s.extend(val.match_indices('a').map(|(i, _)| (i, acc.len())));
            val.find('S').map(|v| s.push((v, acc.len())));
    ...
    let res = bfs(m.as_slice(), s.as_slice(), e).unwrap();
